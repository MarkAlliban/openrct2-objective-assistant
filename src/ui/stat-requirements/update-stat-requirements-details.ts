...
export const updateStatRequirementsDetails = (
  ...
  for (let requirementIndex = 0; requirementIndex < 6; requirementIndex++) {
    ...
    if (visible) {
      ...
      if (actual !== undefined) {
        textWrapped.push(
          `${statsCalculated ? (units[type] ? units[type](actual) : actual) : "?"} (target ${units[type] ? units[type](required) : required})${overridden ? " *" : ""}`,
        );
      }
    }
